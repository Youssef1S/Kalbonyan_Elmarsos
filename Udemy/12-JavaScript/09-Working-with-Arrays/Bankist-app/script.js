...
const calcDisplayBalance = function (acc) {
  ...
  const formatMov = formatCur(acc.balance, acc.locale, acc.currency);
...
const calcDisplaySummary = function (acc) {
  ...
  const formatMov = formatCur(incomes, acc.locale, acc.currency);
...
const startLogOutTimer = function () {
  ...
  const tick = function () {
    ...
    // When 0 seconds, step timer and log out user
    if (time === 0) {
      clearInterval(timer);
